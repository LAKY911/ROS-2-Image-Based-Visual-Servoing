...
class KeyboardListener:
    ...
    def _on_key_release(self, key: Union[Key, KeyCode, None]) -> None:
        if key is None:
            return
        ...
        try:
            if (
                ...
                self._twist_cmd = np.zeros(6)
                if self._command_forward_node.dof:
            ...
            elif key_str in (self._valid_numbers or ""):
                # get index of key_str within valid_numbers
                if self._valid_numbers and self._joint_velocity_cmd is not None:
                    index = self._valid_numbers.index(key_str)
                    if 0 <= index < (self._command_forward_node.dof or 0):
                        self._joint_velocity_cmd[index] = 0.0
            self._command_forward_node.twist_cmd = self._twist_cmd
            if self._joint_velocity_cmd is not None:
    ...
    def _on_key_press(self, key: Union[Key, KeyCode, None]) -> None:
        ...
        try:
            if (
                ...
                self._twist_cmd = np.zeros(6)
                self._command_forward_node.get_logger().info("Pausing all commands.")
                self._joint_velocity_cmd = np.zeros(self._command_forward_node.dof)
            elif (
                key_str
                == self._command_forward_node.keyboard_layout.translation.x.increase
